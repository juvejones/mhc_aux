...
from mhcflurry.class1_panallele.cross_validation import cross_validation_folds
from mhcflurry.class1_panallele.train import (train_across_models_and_folds, 
...
def hyperparameter_selection():
    ...
    models = HYPERPARAMETER_DEFAULTS.models_grid(
        ...
        loss=["binary_crossentropy"],embedding_output_dim=[16,32,64],
        dropout_probability=[.1,.2,.25],n_training_epochs=[250]
        )
    
    return models

def hyperparameter_selection_score_by_cv(models = None, fold=3, 
        data=None, alleles=["pan"]):
    """
